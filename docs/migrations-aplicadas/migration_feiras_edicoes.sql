-- ================================================================
-- Modulo Feiras — modelo (a) aprovado: feiras guarda so a IDENTIDADE (nome),
-- uma feira acontece TODO ANO (as vezes em local diferente) e cada
-- ocorrencia vira uma linha em feira_edicoes (ano/local/datas). Participacao
-- aponta pra uma edicao (edicao_id), nao mais direto pra feira.
--
-- POR QUE: "Expofeira 2028" e "Expofeira 2029" sao a MESMA feira pro
-- usuario. O modelo anterior (uma linha de `feiras` por ano, ano inferido
-- por regex do nome digitado) causou dois bugs seguidos — sempre por
-- inferir estrutura a partir de texto livre. Este modelo tira o ano do
-- texto: UNIQUE(feira_id, ano) existe de verdade no banco, e "ano novo"
-- vira uma edicao nova por CONSTRUCAO (linha em outra tabela), nunca por
-- interpretacao da tela.
--
-- Dados de teste descartados (TRUNCATE) — combinado com o usuario, nao ha
-- producao usando o modulo Feiras ainda.
--
-- Execute no SQL Editor do Supabase, de cima pra baixo, em uma unica sessao.
-- ================================================================

-- ── PASSO 1: descartar dados de teste (schema muda estruturalmente) ──

TRUNCATE public.feira_participacoes, public.feiras;

-- ── PASSO 2: feiras perde local/data — passam a ser da EDICAO ────
...
-- ── PASSO 3: tabela feira_edicoes ─────────────────────────────────
-- Uma linha por ano/ocorrencia de uma feira. `ano` e coluna NORMAL (nao
-- GENERATED) computada em JS a partir de data_inicio no momento do insert —
-- uma coluna GENERATED quebraria o INSERT...SELECT * FROM
-- jsonb_populate_recordset que restaurar_backup_fazenda usa pra todas as
-- tabelas (Postgres nao aceita valor explicito, nem via SELECT *, numa
-- coluna GENERATED). O CHECK abaixo e a rede de seguranca contra o app
-- mandar um `ano` que nao bate com `data_inicio`.

CREATE TABLE public.feira_edicoes (
  ...
  CHECK (ano = EXTRACT(YEAR FROM data_inicio)::int),
  UNIQUE (conta_id, fazenda_id, feira_id, ano)
);

CREATE INDEX idx_feira_edicoes_fazenda ON public.feira_edicoes(conta_id, fazenda_id);
CREATE INDEX idx_feira_edicoes_feira   ON public.feira_edicoes(feira_id);

ALTER TABLE public.feira_edicoes ENABLE ROW LEVEL SECURITY;

CREATE POLICY "feira_edicoes_select" ON public.feira_edicoes
  FOR SELECT USING (tem_acesso_fazenda(fazenda_id));

CREATE POLICY "feira_edicoes_insert" ON public.feira_edicoes
  FOR INSERT WITH CHECK (tem_acesso_fazenda(fazenda_id) AND pode_editar_modulo(conta_id, 'feiras'));

CREATE POLICY "feira_edicoes_update" ON public.feira_edicoes
  FOR UPDATE USING (tem_acesso_fazenda(fazenda_id) AND pode_editar_modulo(conta_id, 'feiras'));

CREATE POLICY "feira_edicoes_delete" ON public.feira_edicoes
  FOR DELETE USING (tem_acesso_fazenda(fazenda_id) AND pode_editar_modulo(conta_id, 'feiras'));

-- ── PASSO 4: feira_participacoes aponta pra EDICAO ────────────────

ALTER TABLE public.feira_participacoes
  DROP COLUMN feira_id,
  ADD COLUMN edicao_id uuid NOT NULL REFERENCES public.feira_edicoes(id) ON DELETE CASCADE;

CREATE INDEX idx_feira_participacoes_edicao ON public.feira_participacoes(edicao_id);

-- ── PASSO 5: excluir_fazenda — adiciona DELETE de feira_edicoes ──
-- Corpo INTEIRO reaplicado (verbatim de migration_feiras_backup_integracao.
-- sql), so com "DELETE FROM public.feira_edicoes" adicionado entre
-- feira_participacoes e feiras (participacoes -> edicoes -> feiras).
...
BEGIN
  ...
  BEGIN
    ...
    DELETE FROM public.feira_edicoes            WHERE fazenda_id = p_fazenda_id;
...
-- ── PASSO 6: restaurar_backup_fazenda — DELETE + INSERT de feira_edicoes ──
-- Corpo INTEIRO reaplicado (verbatim de migration_feiras_backup_integracao.
-- sql), com dois pontos adicionados:
--   - DELETE de feira_edicoes logo apos feira_participacoes, antes de feiras
--     (mesma posicao relativa do INSERT abaixo, invertida).
--   - INSERT de feira_edicoes logo apos feiras, antes de animais.
...
SELECT 'modelo (a) de Feiras (feira_edicoes) aplicado — feiras/feira_participacoes/excluir_fazenda/restaurar_backup_fazenda atualizados com sucesso!' as resultado;
