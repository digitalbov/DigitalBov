...
-- Parte 3/5 — registrar_venda_animais: apos o rateio da receita da venda
-- (inalterado), cria 1 despesa 'Comissao' (se p_comissao > 0) e 1 despesa
-- 'Impostos' (se p_imposto > 0), mesma data/ciclo da venda, cada uma ja
-- rateada na mesma proporcao de valor da venda (ver
-- criar_lancamento_rateado_proporcional, parte 2/5). comissao/imposto
-- continuam gravados em transacoes_animais como ja eram (informativo/auditoria)
-- — isto e ADICIONAL, nao substitui.
...
BEGIN
  ...
  -- proporcao da venda, vinculada a este lancamento via lancamento_origem_id
  -- (o trigger de reversao apaga os dois junto quando a venda for excluida).
...
SELECT 'registrar_venda_animais (com comissao/imposto automaticos) pronta!' as resultado;
