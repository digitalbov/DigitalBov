...
-- Parte 3/3 -- registrar_compra_animais ganha p_frete, mesma ideia da parte
-- 2/3 aplicada a compra: despesa 'Compra de Animais' ja existe; frete vira
-- MAIS UMA despesa separada ('Frete'), rateada na mesma proporcao da compra.
--
-- ATENCAO: mesma mudanca de ASSINATURA da parte 2/3 -- DROP FUNCTION
-- explicito da versao de 10 parametros ANTES do CREATE da versao de 11, pelo
-- mesmo motivo (CREATE OR REPLACE sozinho criaria overload ambiguo).
--
-- Pre-requisito: migration_frete_d5_1.sql ja aplicada (coluna frete existe).
-- Execute no SQL Editor do Supabase.
-- ================================================================

DROP FUNCTION IF EXISTS public.registrar_compra_animais(
...
BEGIN
  ...
  IF p_frete > 0 THEN
    PERFORM public.criar_lancamento_rateado_proporcional(
      ...
      'Frete sobre compra: ' || p_descricao, p_frete, v_lancamento_id, v_transacao_ids
    );
  END IF;
...
GRANT EXECUTE ON FUNCTION public.registrar_compra_animais(
  uuid, uuid, uuid, date, numeric, text, text, numeric, numeric, numeric, jsonb
) TO authenticated;

SELECT 'registrar_compra_animais (com frete) pronta!' as resultado;
