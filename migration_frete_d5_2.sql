...
-- Parte 2/3 -- registrar_venda_animais ganha p_frete, mesmo mecanismo de
-- p_comissao/p_imposto (Bloco D4): grava em transacoes_animais.frete
-- (informativo, so na 1a linha da venda) E cria 1 despesa 'Frete' automatica,
-- rateada na mesma proporcao da venda, vinculada via lancamento_origem_id.
-- A reversao ao excluir o lancamento principal (reverter_venda_ao_excluir_
-- lancamento, Bloco D4 parte 5/5) NAO precisa mudar -- ela ja apaga QUALQUER
-- lancamento com lancamento_origem_id = OLD.id, sem saber o nome do grupo.
--
-- ATENCAO: isto muda a ASSINATURA da funcao (parametro novo no meio da
-- lista) -- ao contrario das migrations anteriores do Bloco D4, que so
-- trocavam o corpo mantendo a assinatura. CREATE OR REPLACE sozinho NAO
-- substitui a funcao de 11 parametros por uma de 12 -- ele cria uma segunda
-- funcao (overload), e chamadas nomeadas via PostgREST ficam ambiguas. Por
-- isso o DROP FUNCTION explicito abaixo, ANTES do CREATE.
--
-- Pre-requisito: migration_frete_d5_1.sql ja aplicada (coluna frete existe).
-- Execute no SQL Editor do Supabase.
-- ================================================================

DROP FUNCTION IF EXISTS public.registrar_venda_animais(
...
BEGIN
  ...
  LOOP
    INSERT INTO public.transacoes_animais (
      ...
      peso_medio, preco_kg, valor_total, contraparte, comissao, imposto, frete, lancamento_id
    ) VALUES (
      ...
      CASE WHEN v_primeiro THEN p_frete    ELSE 0 END,
      v_lancamento_id
    )
  ...
  IF p_comissao > 0 THEN
    PERFORM public.criar_lancamento_rateado_proporcional(
      ...
      'Comissão sobre venda: ' || p_descricao, p_comissao, v_lancamento_id, v_transacao_ids
    );
  END IF;
  ...
  IF p_imposto > 0 THEN
    PERFORM public.criar_lancamento_rateado_proporcional(
      ...
      'Imposto sobre venda: ' || p_descricao, p_imposto, v_lancamento_id, v_transacao_ids
    );
  END IF;

  -- NOVO: frete vira despesa automatica, mesma mecanica de comissao/imposto.
  IF p_frete > 0 THEN
    PERFORM public.criar_lancamento_rateado_proporcional(
      p_conta_id, p_fazenda_id, p_ciclo_id, p_data, 'D', 'Frete',
      'Frete sobre venda: ' || p_descricao, p_frete, v_lancamento_id, v_transacao_ids
    );
  END IF;
...
GRANT EXECUTE ON FUNCTION public.registrar_venda_animais(
  uuid, uuid, uuid, date, numeric, text, text, numeric, numeric, numeric, jsonb, uuid[]
) TO authenticated;

SELECT 'registrar_venda_animais (com frete) pronta!' as resultado;
