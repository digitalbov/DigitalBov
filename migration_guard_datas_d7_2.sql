...
-- Parte 2/2 -- registrar_compra_animais ganha a mesma validacao da parte 1/2,
-- adaptada: aqui nao ha animal_ids (compra CRIA animais), entao o guard olha
-- p_detalhes->>'data_nascimento_estimada' de cada categoria, no MESMO loop que
-- ja validava proprietario_id (nao precisa de loop novo). O client
-- (Financeiro.jsx::salvarCompra) ja bloqueava isto antes de chamar a RPC --
-- isto e so a defesa server-side que faltava, mesmo raciocinio da parte 1/2.
...
BEGIN
  ...
  LOOP
    ...
    -- NOVO: data de nascimento estimada nao pode ser depois da data da compra.
  ...
  LOOP
    ...
    LOOP
      ...
      INSERT INTO public.transacao_animais_itens (conta_id, fazenda_id, transacao_id, animal_id, categoria_venda, proprietario_id, peso_medio, preco_kg, valor) VALUES (p_conta_id, p_fazenda_id, v_transacao_id, v_animal_id, v_categoria, v_proprietario_id, (v_item->>'peso_medio')::numeric, (v_item->>'preco_kg')::numeric, (v_item->>'peso_medio')::numeric * (v_item->>'preco_kg')::numeric);

      INSERT INTO public.pesagens (conta_id, fazenda_id, animal_id, data, tipo, peso_kg, transacao_id) VALUES (p_conta_id, p_fazenda_id, v_animal_id, p_data, 'compra', (v_item->>'peso_medio')::numeric, v_transacao_id);
    END LOOP;
  END LOOP;
...
SELECT 'registrar_compra_animais (com guard de data de nascimento) pronta!' as resultado;
